...
RSpec.describe Api::V1::UsersController, type: :controller do
	...
	context "authenticated and unauthorized users" do
		...
		it "PUT update returns http forbidden" do
			new_user = build(:user)
			put :update, id: my_user.id, user: { name: my_user.name, email: my_user.email, password: my_user.password }
			expect(response).to have_http_status(403)
		end

		it "POST create returns http forbidden" do
			new_user = build(:user)
			post :create, user: { name: my_user.name, email: my_user.email, password: my_user.password }
			expect(response).to have_http_status(403)
		end
	end
	...
	context "authenticated and authorized users" do
		...
		describe "GET index" do
			...
			it "returns ny_user serialized" do
		...
		describe "PUT update" do
			context "with valid attributes" do
				before do
					@new_user = build(:user)
					put :update, id: my_user.id, user: { name: @new_user.name, email: @new_user.email, pawword: @new_user.password, role: "admin" }
				end
